...
export function buildMonitorToolDescription({
  ...
  return (
    ...
    `a rolling debounce; ${delivery} Reports state facts only. A newly failing check, merge conflict, or terminal PR ` +
    "state skips debounce and is reported at the next poll. The monitor itself owns all polling and notifications " +
    ...
    "after a delivered report), status (list this session's monitors), mark_ready (add the ready-for-human-review " +
    "label only " +

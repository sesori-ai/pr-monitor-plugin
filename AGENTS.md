...
```
core/           # Shared core — shell-agnostic, imports nothing from a shell.
...
The split is by dependency direction: `core/` never imports from a shell, so a shell is
only wiring (transport, delivery, config paths). Add a shell as a sibling directory.

## Core flow

1. **Plugin load** — `PrMonitorPlugin({ client, directory, worktree, $ })` (`opencode/index.ts:18`) builds hooks + registers the `pr_monitor` tool. Maintains `watches: Map<"${sessionID} ${owner/repo#n}", Entry>`.
2. **Tool** — `pr_monitor` with `action: start|stop|flush|status` and optional `pr` (`opencode/index.ts:140-188`).
3. **start** — `startWatch` (`opencode/index.ts:87`): parse target → dedupe → `loadConfig` → fetch initial snapshot → reject if not `OPEN` → `new PrWatch(...)` → arm `setInterval(tick, pollIntervalSeconds*1000)`.
4. **tick** — `PrWatch.tick()` (`core/watch.ts:65`): fetch snapshot → `detectActivity` → mark dirty/reset debounce → `maybeAutoFlush()`.
5. **deliver** — `deliver(sessionID, agent)` (`opencode/index.ts:64`) calls `client.session.promptAsync({ path:{id}, body:{agent, model, parts:[{type:"text", text}]} })`. `promptAsync` never rejects on server error — check `result.error`. All messages are Markdown, prefixed `[PR Monitor]` with a clickable `[owner/repo#n](url)` link.
...
- **Debounce + CI-hold** in `maybeAutoFlush` (`core/watch.ts:139`): wait `debounceMinutes` of quiet after activity; if CI is running on an OPEN PR, hold up to `maxCiWaitMinutes` before flushing.
...
- **Failure handling** — `handlePollFailure` (`core/watch.ts:124`): notFound → stop with notice; 10 consecutive poll failures → stop. 10 consecutive delivery failures → stop. Delivery failures roll back the baseline so the same activity is retried.
- **Reentrancy guard** — `ticking` flag prevents overlapping polls.
- **Reload takeover** — `globalThis.__sesoriPrMonitorTakeovers` (`opencode/index.ts:37`) kills zombie timers from prior plugin instances; old watches send one factual stop notice.
...
File: `.opencode/pr-monitor.json` (searched in `directory`, then `worktree`). Loaded fresh per `startWatch`. See `core/config.ts`.

`MonitorConfig` (`core/config.ts:7`):
...
| `announceOnStart` | true | Deliver a full status report immediately on start (`PrWatch.announceInitial`, `core/watch.ts`) so the session sees outstanding items. Reports against baseline 0, then advances baseline to the initial snapshot. |
...
- `createGhRunner($)` (`core/github.ts:39`) wraps Bun `$` shell: ``$`gh ${args}`.quiet().nothrow()``, throws `PollError(msg, { notFound })` on non-zero exit.
- Single GraphQL doc `PR_QUERY` (`core/github.ts:51`) fetches title, url, state, mergeable, head SHA, latest commit's check rollup, review requests/latestReviews, review threads + comments, issue comment count.
- `normalizeSnapshot` (`core/github.ts:111`) → `PrSnapshot`; `ciPhase` → `none|running|concluded`.

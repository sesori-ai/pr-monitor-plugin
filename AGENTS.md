...
- **Stop/mutation fencing** — a stopped watch rejects queued flush/ready actions and remains registered until an
  already-started label mutation drains. Session cleanup and OpenCode reload takeover await that drain before a
  successor can own the same PR.
...
- **Automatic readiness** (`core/readiness.ts`, `core/watch.ts`) — adds the label after green/no CI, definite
  mergeability, and prefixed local replies on every feedback channel. A later head, relevant comment/summary, CI
  regression, or conflict withdraws it urgently. Resolution state, stale review state, pending reviewers, draft,
  and terminal state do not withdraw it.

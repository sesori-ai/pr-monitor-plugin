...
2. **tick** — `PrWatch.tick()` (`core/watch.ts`): fetch snapshot → `detectActivity` → mark dirty/reset debounce → check `hasNewCiFailure` (marks the flush urgent) → `maybeAutoFlush()`.
...
- **Instant CI-failure flush** (`config.flushOnCiFailure`, default on) — the one path that bypasses both timers. `hasNewCiFailure(prev, next)` (`core/activity.ts`) is true when `next` has a failing check `prev` did not (any failure counts when the head SHA changed); `pollOnce` then sets `dirty` **and** `urgent`, and `maybeAutoFlush` skips the quiet window and the CI hold when `urgent`. It marks `dirty` itself because a mid-suite failure is deliberately not `detectActivity` — on the running-suite path nothing else would. `urgentFlushedSha` caps the instant path at one report per head commit (a matrix going red job by job would otherwise wake the session once per job; the stragglers ride along with the debounced suite-conclusion report, and the next push re-arms it). `flush()` clears `urgent`; a delivery failure restores it, so the instant report retries instantly instead of falling back to the debounce.
- **detectActivity** (`core/activity.ts`): state, mergeable, reviews, unresolved threads, inline comments, issue comments, and CI **conclusion** count as activity. CI transitions into "running" and per-check progress do **NOT** (per-check *failures* are handled by the instant-flush path above, not by widening this). Mergeability is compared against the last *definite* (MERGEABLE/CONFLICTING) value — tracked by `PrWatch.lastDefiniteMergeable` — so transient `UNKNOWN` churn from base-branch merges stays quiet while a real `MERGEABLE -> UNKNOWN -> CONFLICTING` settle is still caught.
...
- **Reentrancy guard** — `runExclusive` (per-watch promise queue) serializes `tick()` and `manualFlush()` so overlapping fetches can't apply out of order; ticks skip while an op is pending, manual flushes queue. Auto-flush delivery is **awaited inside** that op, not fire-and-forget: its failure path rolls back exactly the state a later flush advances (`lastFlushAt`, `dirty`, `holdStartedAt`, `urgent`), so a late rejection from an overlapping delivery would otherwise rewind a newer report's baseline and re-fire it immediately. Ticks skipping while a report is in flight is the intended consequence. `tick` also re-checks `stopped` after its awaited fetch so a concurrent `stop()` prevents any late apply/deliver.
...
- Single GraphQL doc `PR_QUERY` (`core/github.ts`) fetches title, url, state, mergeable, head SHA, latest commit's check rollup, review requests/latestReviews, review threads + comments, issue comment count, labels.
...
- A release uses one version for both targets. The root package publishes the OpenCode target as public npm package `@sesori/pr-monitor-opencode` (the `files` allowlist ships only `core/` + `opencode/`), while an annotated `vX.Y.Z` Git tag releases the Claude Code target. Before releasing: update `package.json`, `package-lock.json`, `claude-code/.claude-plugin/plugin.json`, and `CHANGELOG.md`; run `npm run typecheck`, `npm run build`, and `npm run pack:check`; commit the rebuilt bundle; then, from a clean release commit on `main`, run `npm publish`, create the local tag, and push the tag only after publication succeeds.

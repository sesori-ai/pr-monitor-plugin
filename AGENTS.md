...
pi/                  # @sesori/pr-monitor-pi workspace shared by upstream Pi and OMP.
  ...
  omp.ts        # Thin OMP entry; adds post-switch cleanup and resources_discover.
...
- **Pi-family lifecycle** — upstream Pi tears down the old extension instance and emits post-success `session_shutdown` on new/resume/fork/reload, so common cleanup lives there. OMP retains its extension runner across new/resume/fork and instead uses the thin `pi/omp.ts` post-success `session_switch` handler. Neither adapter clears on cancelable before-events.

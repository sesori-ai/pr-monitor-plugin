...
`pr-monitor.json`, loaded fresh per start/action: all adapters first look for repository `.pr-monitor.json`. OpenCode then falls back to `<directory>/.opencode/` and `<worktree>/.opencode/`; Claude Code falls back to `<project>/.claude/`, then `.opencode/`; trusted Pi/OMP projects use `${CONFIG_DIR_NAME}/pr-monitor.json`, then `.opencode/pr-monitor.json`. OMP's
compatibility shim resolves that imported constant to `.omp`; do not replace it with a hardcoded host branch.
`MonitorConfig` contains common settings; `ClaudeMonitorConfig` adds `desktopNotifications`, `keepAlive`, and `keepAliveMaxMinutes`. Loading is permissive: unknown keys ignored, invalid values dropped, invalid JSON logged, missing file → defaults.
